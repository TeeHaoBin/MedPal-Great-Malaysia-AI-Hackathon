...
ECR_REPOSITORY_NAME="gcp-vision-ocr-lambda"
LAMBDA_FUNCTION_NAME="gcp-vision-ocr-processor"
IAM_ROLE_NAME="LambdaGCPVisionOCRRole"
IAM_POLICY_NAME="LambdaGCPVisionOCRPolicy"
...
# GCP Credentials (base64 encoded JSON file)
...
if [ -z "$GCP_CREDENTIALS_BASE64" ]; then
  echo "Please provide your GCP service account credentials as base64 encoded JSON."
  echo "You can encode your GCP credentials file using: base64 -i your-gcp-credentials.json"
  read -p "Enter base64 encoded GCP credentials: " GCP_CREDENTIALS_BASE64
fi
...
echo "--- Starting Google Cloud Vision OCR Deployment ---"
...
aws lambda update-function-configuration \
  --function-name $LAMBDA_FUNCTION_NAME \
  --environment "Variables={GCP_CREDENTIALS_BASE64=$GCP_CREDENTIALS_BASE64}" \
  --region $AWS_REGION

# 8. Add S3 trigger permissions
echo "Adding S3 trigger permissions to Lambda..."
# Remove old permissions to avoid conflicts, then add the new one.
aws lambda remove-permission --function-name $LAMBDA_FUNCTION_NAME --statement-id "S3-Invoke-Permission-GCP-$(date +%s)" --region $AWS_REGION 2>/dev/null || true
aws lambda add-permission \
  --function-name $LAMBDA_FUNCTION_NAME \
  --statement-id "S3-Invoke-Permission-GCP-$(date +%s)" \
...
NOTIFICATION_CONFIG=$(cat <<EOF
{
    "LambdaFunctionConfigurations": [
        {
            "Id": "gcp-vision-ocr-trigger-config",
...
echo "The Google Cloud Vision OCR system is now live!"
echo "Upload a PDF to 's3://$S3_BUCKET_NAME/$S3_PREFIX' to trigger the OCR process."
echo ""
echo "Next steps:"
echo "1. Test by uploading a PDF to the S3 bucket"
echo "2. Check CloudWatch logs for the Lambda function"
echo "3. Verify results in DynamoDB table: $DYNAMODB_TABLE_NAME"