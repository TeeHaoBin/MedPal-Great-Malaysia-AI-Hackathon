...
function SidebarContent({ onSessionSelect, activeSessionId, onSessionDeleted }: MedicalSidebarProps) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-x-hidden overflow-y-auto">
      ...
      {/* Chat Sessions */}
      <div className="mt-6">
        <div className="px-2 py-1 text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider flex items-center space-x-2">
          <History className="w-3 h-3" />
          {(animate ? open : true) && <span>Chat History</span>}
        </div>
        <div className="mt-2 space-y-1">
          {loading ? (
            (animate ? open : true) && <div className="text-xs text-neutral-500 px-2">Loading...</div>
          ) : sessions.length === 0 ? (
            null
          ) : (
            sessions.map((session) => (
              <div
                ...
                className={cn(
                  ...
                  activeSessionId === session.sessionId && "bg-neutral-200 dark:bg-neutral-700"
                )}
              >
                <div className="flex items-center space-x-2 min-w-0 flex-1">
                  <IconMessageCircle className="h-4 w-4 shrink-0 text-neutral-600 dark:text-neutral-300" />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center space-x-1 mt-1">
                      ...
                      {session.hasSensitiveData && (
                        <IconShield className="h-3 w-3 text-amber-500" />
                      )}
                    </div>
                  </div>
                </div>
...
export function MedicalSidebar({ onSessionSelect, activeSessionId, onSessionDeleted }: MedicalSidebarProps) {
  return (
